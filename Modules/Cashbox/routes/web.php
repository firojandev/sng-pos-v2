...
Route::middleware(['auth', 'feature:cashbox'])->prefix('cashbox')->name('cashbox.')->group(function () {
    ...
    Route::post('/cash-in', [CashboxController::class, 'cashIn'])->name('cash-in')->middleware('permission:cashbox.cash-in');
    Route::post('/cash-out', [CashboxController::class, 'cashOut'])->name('cash-out')->middleware('permission:cashbox.cash-out');
});
