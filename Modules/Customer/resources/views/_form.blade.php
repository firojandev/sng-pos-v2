...
<div class="field">
    ...
    <div class="seg">
        <button type="button" class="status-btn {{ old('status', $customer->status ?? 'active') === 'active' ? 'active' : '' }}" data-val="active" onclick="selCustomerStatus(this)">
            <span class="bn">সক্রিয়</span><span class="en">Active</span>
        </button>
        <button type="button" class="status-btn {{ old('status', $customer->status ?? 'active') === 'inactive' ? 'active' : '' }}" data-val="inactive" onclick="selCustomerStatus(this)">
            <span class="bn">নিষ্ক্রিয়</span><span class="en">Inactive</span>
        </button>
    </div>
    <input type="hidden" name="status" id="status-input" value="{{ old('status', $customer->status ?? 'active') }}">
</div>

<script>
    function selCustomerStatus(btn) {
