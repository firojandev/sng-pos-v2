        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('from_account', function (AccountTransfer $transfer) {
                ...
                $header = '<div style="font-weight:700; color:var(--red-600); font-size:13px; display:flex; align-items:center; gap:6px;">'
                    ...
                    .'</span>'
                    .'<span style="max-width:150px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="'.e($acc->name).'">'.e($acc->name).'</span>'
                    .'</div>';

                $typeBadges = [
                    'cash' => '<span style="display:inline-block; font-size:10.5px; padding:2px 6px; border-radius:4px; font-weight:600; background:var(--green-100); color:var(--green-ink); white-space:nowrap;"><span class="bn">নগদ</span><span class="en" style="display:none;">Cash</span></span>',
                    'bank' => '<span style="display:inline-block; font-size:10.5px; padding:2px 6px; border-radius:4px; font-weight:600; background:var(--blue-100); color:var(--blue-ink); white-space:nowrap;"><span class="bn">ব্যাংক</span><span class="en" style="display:none;">Bank</span></span>',
                    'mfs' => '<span style="display:inline-block; font-size:10.5px; padding:2px 6px; border-radius:4px; font-weight:600; background:var(--gold-100); color:var(--gold-ink); white-space:nowrap;"><span class="bn">মোবাইল ব্যাংকিং</span><span class="en" style="display:none;">MFS</span></span>',
                ];
            ...
            ->addColumn('details', function (AccountTransfer $transfer) {
                $note = $transfer->note
                    ? '<div style="font-size:12.5px; color:var(--ink-800); max-width:180px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; word-break:break-word;" title="'.e($transfer->note).'">'.e($transfer->note).'</div>'
                    : '<div style="font-size:12px; color:var(--ink-400);">—</div>';

                $creator = e($transfer->creator->name ?? 'System');
                $creatorHtml = '<div style="font-size:11px; color:var(--ink-500); margin-top:2px; display:flex; align-items:center; gap:4px; max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="'.e($transfer->creator->name ?? 'System').'">'
