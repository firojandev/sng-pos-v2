<x-core::layout
    title="প্ল্যান"
    title-en="Plans"
    subtitle="সাবস্ক্রিপশন প্ল্যানসমূহ পরিচালনা করুন"
    subtitle-en="Manage subscription plans"
    active="plans"
>
    <div class="section-row">
        <div class="filters"></div>
        <a class="btn btn-gold" href="{{ route('plans.create') }}">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="#fff" stroke-width="2.2" stroke-linecap="round"/></svg>
            <span class="bn">নতুন প্ল্যান</span><span class="en">New Plan</span>
        </a>
    </div>

    <div class="panel" style="margin-top:0;">
        <div class="panel-body">
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th class="bn">প্ল্যান</th><th class="en" style="display:none;">Plan</th>
                            <th class="bn">মূল্য</th><th class="en" style="display:none;">Price</th>
                            <th class="bn">ইউজার</th><th class="en" style="display:none;">Users</th>
                            <th class="bn">শাখা</th><th class="en" style="display:none;">Branches</th>
                            <th class="bn">গুদাম</th><th class="en" style="display:none;">Warehouses</th>
                            <th class="bn">পণ্য</th><th class="en" style="display:none;">Products</th>
                            <th class="bn">সাবস্ক্রাইবার</th><th class="en" style="display:none;">Subscribers</th>
                            <th class="bn">অবস্থা</th><th class="en" style="display:none;">Status</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($plans as $plan)
                            <tr>
                                <td class="cell-main">{{ $plan->name }}</td>
                                <td>৳{{ number_format($plan->price, 0) }}/{{ $plan->billing_cycle === 'monthly' ? 'মাস' : 'বছর' }}</td>
                                <td>{{ $plan->max_users ?? '∞' }}</td>
                                <td>{{ $plan->max_branches ?? '∞' }}</td>
                                <td>{{ $plan->max_warehouses ?? '∞' }}</td>
                                <td>{{ $plan->max_products ?? '∞' }}</td>
                                <td>{{ $plan->subscriptions_count }}</td>
                                <td>
                                    @if ($plan->status === 'active')
                                        <span class="badge b-green bn">সক্রিয়</span><span class="badge b-green en" style="display:none;">Active</span>
                                    @else
                                        <span class="badge b-grey bn">নিষ্ক্রিয়</span><span class="badge b-grey en" style="display:none;">Inactive</span>
                                    @endif
                                </td>
                                <td>
                                    <div class="row-actions">
                                        <a class="act" title="Edit" href="{{ route('plans.edit', $plan) }}">
                                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z" stroke="#5C6B65" stroke-width="1.5" stroke-linejoin="round"/></svg>
                                        </a>
                                        <form method="POST" action="{{ route('plans.destroy', $plan) }}" onsubmit="return confirm('এই প্ল্যান মুছে ফেলতে চান?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="act" title="Delete">
                                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M9 7V4h6v3m-8 0 1 13h8l1-13" stroke="#C1443C" stroke-width="1.5" stroke-linejoin="round"/></svg>
                                            </button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="9"><div class="helper" style="margin-top:0;">কোনো প্ল্যান নেই</div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</x-core::layout>
