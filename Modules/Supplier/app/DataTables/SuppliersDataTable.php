...
class SuppliersDataTable extends BaseDataTable
{
        ...
        return (new EloquentDataTable($query))
            ->editColumn('name', function (Supplier $supplier) {
                ...
                $code = $supplier->supplier_code
                    ? '<div style="font-size:11.5px; color:var(--ink-500); font-family:var(--font-mono, monospace);">#'.e($supplier->supplier_code).'</div>'
                    : '';

                return '<div style="display:flex; align-items:center; gap:10px;">'
                    .'<div style="width:34px; height:34px; border-radius:8px; background:var(--gold-100); color:var(--gold-ink); font-weight:700; font-size:13px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">'
                    .e($initial)
                    .'</div>'
            ...
            ->addColumn('contact', function (Supplier $supplier) {
                ...
                $email = $supplier->email
                    ? '<div style="font-size:11.5px; color:var(--ink-500); display:flex; align-items:center; gap:5px; margin-top:2px; max-width:160px;">'
                        ...
                        .'<span style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="'.e($supplier->email).'">'.e($supplier->email).'</span>'
                        .'</div>'
                    : '';
            ...
            ->editColumn('address', function (Supplier $supplier) {
                ...
                return '<div style="font-size:12px; color:var(--ink-700); display:flex; align-items:flex-start; gap:5px; max-width:200px; line-height:1.4;">'
                    ...
                    .'<span style="min-width:0; flex:1; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; word-break:break-word;" title="'.e($supplier->address).'">'
                    .e($supplier->address)
                    .'</span>'
                    .'</div>';
            })
            ->addColumn('purchase_summary', function (Supplier $supplier) {
                ...
                if ($totalCount === 0) {
                    ...
                    $dueBadge = '<span style="display:inline-block; font-size:10.5px; font-weight:600; padding:1px 6px; border-radius:4px; background:var(--blue-100); color:var(--blue-ink); margin-left:4px; white-space:nowrap;">বিল পরিশোধিত</span>';
                }
    ...
    public function query(Supplier $model): QueryBuilder
    {
        ...
        return $query->latest('suppliers.id');
    }
